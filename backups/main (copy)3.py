...
def newuser_submit():
  ...
  newuser_window.geometry("300x240")
  newuser_window.configure(bg="white")
  ...
  genders = ["Male", "Female"]
  gender_entry = ttk.Combobox(newuser_window, values=genders, width=13)
  ...
  dob_year_entry = Entry(newuser_window, width=4)
  dob_month_entry = Entry(newuser_window, width=4)
  dob_day_entry = Entry(newuser_window, width =4)
  dob_year_entry.grid(row=5, column=3)
  dob_month_entry.grid(row=5, column=2)
  dob_day_entry.grid(row=5, column=1)

  '''#age entry
  age_label = Label(newuser_window, text="Age:", bg="white")
  age_label.grid(row=6, column=0)
  age_entry = Entry(newuser_window, width=15, bg="grey")
  age_entry.config(state="disable")
  age_entry.grid(row=6, column=1, columnspan=3)
  #age_entry.insert(calculate_age(yearborn, monthborn, dayborn))'''
  ...
  #create maintenance calories entry
  maintenance_label = Label(newuser_window, text="Maintenance calories:", bg="white")
  maintenance_label.grid(row=9, column=0)
  maintenance_entry = Entry(newuser_window, width=15, bg="grey")
  maintenance_entry.config(state="disable")
  maintenance_entry.grid(row=9, column=1, columnspan=3)

  #calculate maintenance calories
  maintenance_calories = 10 * weight_entry.get() + 6.25 * height_entry.get() - 5 * calculate_age()
  maintenance_label.insert(maintenance_calories)
  ...
  submitaccount_button.grid(row=10,column=0, columnspan=2)

  submitage_button = Button(newuser_window, text="Submitage", command=lambda:calculate_age(dob_day_entry.get(), dob_month_entry.get(), dob_year_entry.get()))
  submitage_button.grid(row=10,column=1, columnspan=2)

def calculate_age(yearborn, monthborn, dayborn):
    yearborn = int(yearborn)
    monthborn = int(monthborn)
    dayborn = int(dayborn)
    birthDate = date(dayborn, monthborn, yearborn)
    today = date.today()
...
def createaccount(newuser_window, password, firstname, lastname, dob, height, weight):
  ...
  if password == "":
    ...
    with database_connection:
      sql_statement = "INSERT INTO TblUsers (First_Name, Last_Name, Gender, Weight, Height, Date_Of_Birth, Password) VALUES ('%s', '%s', '%s' %s, %s, '%s', '%s')" % (firstname, lastname, weight, height, dob, password)
...
def add_meal(meal_record, mealbox, calseatenbox, hourselect, minuteselect):
  ...
  time_eaten = hourselect.get() + minuteselect.get()
  todays_date = datetime.today().strftime('%Y-%m-%d')
  print(user_logged_in, mealbox.get(), todays_date, time_eaten, calseatenbox.get())
  with database_connection:
    sql_statement = "INSERT INTO TblMeals (UserID, Mealdesc, Dateeaten, Timeeaten, Calories) VALUES (%s, '%s', '%s', '%s', %s)" % (user_logged_in, mealbox.get(), todays_date, time_eaten, calseatenbox.get())
    database_cursor.execute(sql_statement)
    database_connection.commit()  
